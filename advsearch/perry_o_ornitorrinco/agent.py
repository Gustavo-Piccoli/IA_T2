...
class MinMax:
    ...
    def __max_play(self,node:Node):
        ...
        if node.depth >= self.max_depth:
            node.value = max(node.value,self.heuristic.get_state_value(node))
            return node.value
        move = None
        successors = []
    ...
    def __min_play(self,node:Node):
        ...
        if node.board.is_terminal_state():
            if node.board.winner() == node.color:
                ...
                return node.value
        if node.depth >= self.max_depth:
            node.value = min(node.value,self.heuristic.get_state_value(node))
            return node.value
        move = None
        successors = []
        for move in node.board.legal_moves(node.color):
    ...
    def quiesce(self,alpha, beta, node:Node):

        """
         most obvious ones: forced moves (i.e. there is only one move), corner moves and moves to a square adjacent to a corne
        """
        ...
        for move in legal_moves:
            if len(legal_moves) <= 1 or self.__corner_move(move):
                succ = self.__create_successor(node,move)
                score = -self.quiesce( -beta, -alpha, succ)
                if score >= beta:
                    return beta
                if score > alpha:
                    alpha = score
    ...
    def __corner_move(self,move):
        ...
        if move == (0,1) or move == (1,0) or move == (1,1):
            return True
        if move == (0,6) or move == (1,7) or move == (1,6):
            return True
        if move == (7,1) or move == (6,0) or move == (6,1):
            return True
        if move == (7,6) or move == (6,7) or move == (6,6): 
            return True
        return False 
